...
import { PublicKeyHash, InstrumentCode, TxId } from './protocol_types';
...
export class InstrumentReceiver {
// The address receiving the tokens
	address = new PublicKeyHash(); // PublicKeyHash `json:"address,omitempty"`
// Number of tokens to be received by address at Output X
	quantity; // uint64 `json:"quantity,omitempty"`
// 0 = No Oracle-signed Message (OracleConfirmationSig skipped in
	// serialization), 1 = ECDSA+secp256k1. If the contract for the instrument
...
// Length 0-255 bytes. If restricted to a oracle (whitelist) or has
	...
	// restrictions(trade restriction/age restriction fields in the Instrument Type
...
// Serialize returns the byte representation of the message.
	...
	Equal(other: InstrumentReceiver): boolean {
...
// transfer.
export class InstrumentSettlement {
...
// Three letter character that specifies the instrument type. Example: COU
	instrument_type; // string `json:"instrument_type,omitempty"`
// A unique code that is used to identify the instrument. It is generated by
	...
	instrument_code = new InstrumentCode(); // InstrumentCode `json:"instrument_code,omitempty"`
...
// Serialize returns the byte representation of the message.
	Serialize(): Buffer {
		...
		{
			if (!this.settlements) this.settlements = [];
	...
	Validate() {
		...
		// instrument_code (InstrumentCode)
		{
			...
			if (err) {
				return sprintf('field instrument_code is invalid : %s', err);
			}
		}
	...
	Equal(other: InstrumentSettlement): boolean {
...
export class InstrumentTransfer {
...
// Each element has the value of tokens to be spent from the input
	...
	instrument_senders = []; // []QuantityIndex `json:"instrument_senders,omitempty"`
// Each element has the value of tokens to be received, the address, and
	// an oracle signature if required.
	instrument_receivers = []; // []InstrumentReceiver `json:"instrument_receivers,omitempty"`
// Serialize returns the byte representation of the message.
	Serialize(): Buffer {
		const buf = new _.Writer();
		...
		{
			if (!this.instrument_senders) this.instrument_senders = [];
			WriteVariableSize(buf, this.instrument_senders.length, 8, 8);
		...
		{
			if (!this.instrument_receivers) this.instrument_receivers = [];
			WriteVariableSize(buf, this.instrument_receivers.length, 8, 8);
	...
	Validate() {
		...
		{
			if (this.instrument_senders.length > (2 ** 8) - 1) {
				return sprintf('list field instrument_senders has too many items %d/%d', this.instrument_senders.length, (2 ** 8) - 1);
			}

			this.instrument_senders.forEach((value, i) => {
				const err = value.Validate();
				if (err) {
					return sprintf('list field instrument_senders[%d] is invalid : %s', i, err);
				}
			});
		}
		...
		{
			if (this.instrument_receivers.length > (2 ** 8) - 1) {
				return sprintf('list field instrument_receivers has too many items %d/%d', this.instrument_receivers.length, (2 ** 8) - 1);
			}

			this.instrument_receivers.forEach((value, i) => {
				const err = value.Validate();
				if (err) {
					return sprintf('list field instrument_receivers[%d] is invalid : %s', i, err);
				}
			});
		}
		return;
	...
	Equal(other: InstrumentTransfer): boolean {
...
// Serialize returns the byte representation of the message.
	Serialize(): Buffer {
		...
		{
			if (!this.administration) this.administration = [];
		...
		// Management ([]Manager)
		{
			if (!this.management) this.management = [];
...
// Where an instrument has a vote multiplier, true must be set here for the
