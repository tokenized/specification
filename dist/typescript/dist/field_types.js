...
class Administrator {
    ...
    Validate() {
        ...
        // name (string)
        {
            if (this.name && this.name.length > (Math.pow(2, 8)) - 1) {
                return sprintf_js_1.sprintf('varchar field name too long %d/%d', this.name.length, (Math.pow(2, 8)) - 1);
            }
        }
        return;
    }
...
// field in a Contract or Instrument, as defined in the ContractFormation and
// InstrumentCreation messages.
class Amendment {
    // Serialize returns the byte representation of the message.
    Serialize() {
        const buf = new util_1.default.Writer();
    ...
    Validate() {
        ...
        // data ([]byte)
        {
            if (this.data && this.data.length > (Math.pow(2, 32)) - 1) {
                return sprintf_js_1.sprintf('varbin field data too long %d/%d', this.data.length, (Math.pow(2, 32)) - 1);
            }
        }
        return;
    }
...
// InstrumentReceiver An InstrumentReceiver is a quantity, address, and oracle
// signature. The quantity could be used to describe a number of tokens, or
// a value. The address is where the instrument will be sent.
class InstrumentReceiver {
    ...
    Validate() {
        ...
        {
            if (this.oracle_confirmation_sig && this.oracle_confirmation_sig.length > (Math.pow(2, 8)) - 1) {
                return sprintf_js_1.sprintf('varbin field oracle_confirmation_sig too long %d/%d', this.oracle_confirmation_sig.length, (Math.pow(2, 8)) - 1);
            }
        }
...
exports.InstrumentReceiver = InstrumentReceiver;
// InstrumentSettlement InstrumentSettlement is the data required to settle an instrument
// transfer.
class InstrumentSettlement {
    constructor() {
        ...
        this.instrument_code = new protocol_types_1.InstrumentCode(); // InstrumentCode `json:"instrument_code,omitempty"`
        // Each element contains the resulting token balance of Instrument X for the
    ...
    Validate() {
        ...
        // settlements ([]QuantityIndex)
        {
            if (this.settlements.length > (Math.pow(2, 8)) - 1) {
                return sprintf_js_1.sprintf('list field settlements has too many items %d/%d', this.settlements.length, (Math.pow(2, 8)) - 1);
            }
    ...
    Equal(other) {
        ...
        // InstrumentType (string)
        if (this.instrument_type !== other.instrument_type) {
            return false;
        }
        // InstrumentCode (InstrumentCode)
        if (!this.instrument_code.Equal(other.instrument_code)) {
            return false;
        }
        // Settlements ([]QuantityIndex)
...
exports.InstrumentSettlement = InstrumentSettlement;
// InstrumentTransfer InstrumentTransfer is the data required to transfer an instrument.
class InstrumentTransfer {
    constructor() {
        // A unique code that is used to identify the instrument. It is generated by
        // hashing the contract public key hash and the instrument index.
        // SHA256(contract PKH + instrument index)
        ...
        this.instrument_senders = []; // []QuantityIndex `json:"instrument_senders,omitempty"`
        // Each element has the value of tokens to be received, the address, and
        // an oracle signature if required.
        this.instrument_receivers = []; // []InstrumentReceiver `json:"instrument_receivers,omitempty"`
    }
    // Serialize returns the byte representation of the message.
    Serialize() {
        const buf = new util_1.default.Writer();
        ...
        // InstrumentSenders ([]QuantityIndex)
        {
            if (!this.instrument_senders)
                this.instrument_senders = [];
            bytes_1.WriteVariableSize(buf, this.instrument_senders.length, 8, 8);
            this.instrument_senders.forEach(value => {
                buf.write(value.Serialize());
            });
        }
        // InstrumentReceivers ([]InstrumentReceiver)
        {
            if (!this.instrument_receivers)
                this.instrument_receivers = [];
            bytes_1.WriteVariableSize(buf, this.instrument_receivers.length, 8, 8);
            this.instrument_receivers.forEach(value => {
                buf.write(value.Serialize());
            });
        }
        return buf.buf;
    ...
    Write(buf) {
        ...
        {
            const size = bytes_1.ReadVariableSize(buf, 8, 8);
            this.instrument_senders = [...Array(size)]
                .map(() => {
                const newValue = new QuantityIndex();
                newValue.Write(buf);
                return newValue;
        ...
        {
            const size = bytes_1.ReadVariableSize(buf, 8, 8);
            this.instrument_receivers = [...Array(size)]
                .map(() => {
                const newValue = new InstrumentReceiver();
                newValue.Write(buf);
                return newValue;
            });
        }
    ...
    Validate() {
        ...
        // instrument_senders ([]QuantityIndex)
        {
            if (this.instrument_senders.length > (Math.pow(2, 8)) - 1) {
                return sprintf_js_1.sprintf('list field instrument_senders has too many items %d/%d', this.instrument_senders.length, (Math.pow(2, 8)) - 1);
            }
            this.instrument_senders.forEach((value, i) => {
                const err = value.Validate();
                if (err) {
                    return sprintf_js_1.sprintf('list field instrument_senders[%d] is invalid : %s', i, err);
                }
            });
        }
        // instrument_receivers ([]InstrumentReceiver)
        {
            if (this.instrument_receivers.length > (Math.pow(2, 8)) - 1) {
                return sprintf_js_1.sprintf('list field instrument_receivers has too many items %d/%d', this.instrument_receivers.length, (Math.pow(2, 8)) - 1);
            }
            this.instrument_receivers.forEach((value, i) => {
                const err = value.Validate();
                if (err) {
                    return sprintf_js_1.sprintf('list field instrument_receivers[%d] is invalid : %s', i, err);
                }
            });
        }
        return;
    ...
    Equal(other) {
        ...
        // InstrumentSenders ([]QuantityIndex)
        if (this.instrument_senders.length !== 0 || other.instrument_senders.length !== 0) {
            if (this.instrument_senders.length !== other.instrument_senders.length) {
                return false;
            }
            if (!this.instrument_senders.every((value, i) => {
                if (!value.Equal(other.instrument_senders[i])) {
                    return false;
                }
                return true;
            }))
        ...
        // InstrumentReceivers ([]InstrumentReceiver)
        if (this.instrument_receivers.length !== 0 || other.instrument_receivers.length !== 0) {
            if (this.instrument_receivers.length !== other.instrument_receivers.length) {
                return false;
            }
            if (!this.instrument_receivers.every((value, i) => {
                if (!value.Equal(other.instrument_receivers[i])) {
                    return false;
                }
                return true;
            }))
...
exports.InstrumentTransfer = InstrumentTransfer;
...
class Document {
    ...
    Validate() {
        ...
        // type (string)
        {
            if (this.type && this.type.length > (Math.pow(2, 8)) - 1) {
        ...
        // contents ([]byte)
        {
            if (this.contents && this.contents.length > (Math.pow(2, 32)) - 1) {
                return sprintf_js_1.sprintf('varbin field contents too long %d/%d', this.contents.length, (Math.pow(2, 32)) - 1);
            }
        }
        return;
    }
...
class Entity {
    ...
    Validate() {
        ...
        // unit_number (string)
        if (this.address_included) {
            if (this.unit_number && this.unit_number.length > (Math.pow(2, 8)) - 1) {
                return sprintf_js_1.sprintf('varchar field unit_number too long %d/%d', this.unit_number.length, (Math.pow(2, 8)) - 1);
            }
        }
        // building_number (string)
        if (this.address_included) {
            if (this.building_number && this.building_number.length > (Math.pow(2, 8)) - 1) {
                return sprintf_js_1.sprintf('varchar field building_number too long %d/%d', this.building_number.length, (Math.pow(2, 8)) - 1);
            }
        }
        // street (string)
        if (this.address_included) {
            if (this.street && this.street.length > (Math.pow(2, 16)) - 1) {
                return sprintf_js_1.sprintf('varchar field street too long %d/%d', this.street.length, (Math.pow(2, 16)) - 1);
            }
        }
        // suburb_city (string)
        if (this.address_included) {
            if (this.suburb_city && this.suburb_city.length > (Math.pow(2, 8)) - 1) {
                return sprintf_js_1.sprintf('varchar field suburb_city too long %d/%d', this.suburb_city.length, (Math.pow(2, 8)) - 1);
            }
        }
        ...
        {
            if (this.email_address && this.email_address.length > (Math.pow(2, 8)) - 1) {
                return sprintf_js_1.sprintf('varchar field email_address too long %d/%d', this.email_address.length, (Math.pow(2, 8)) - 1);
            }
        }
        // phone_number (string)
        {
            if (this.phone_number && this.phone_number.length > (Math.pow(2, 8)) - 1) {
                return sprintf_js_1.sprintf('varchar field phone_number too long %d/%d', this.phone_number.length, (Math.pow(2, 8)) - 1);
            }
        }
        // administration ([]Administrator)
        {
            if (this.administration.length > (Math.pow(2, 8)) - 1) {
                return sprintf_js_1.sprintf('list field administration has too many items %d/%d', this.administration.length, (Math.pow(2, 8)) - 1);
            }
        ...
        // management ([]Manager)
        {
            if (this.management.length > (Math.pow(2, 8)) - 1) {
                return sprintf_js_1.sprintf('list field management has too many items %d/%d', this.management.length, (Math.pow(2, 8)) - 1);
            }
...
class Oracle {
    ...
    Validate() {
        ...
        // url (string)
        {
            if (this.url && this.url.length > (Math.pow(2, 8)) - 1) {
                return sprintf_js_1.sprintf('varchar field url too long %d/%d', this.url.length, (Math.pow(2, 8)) - 1);
            }
        }
        // public_key ([]byte)
        {
            if (this.public_key && this.public_key.length > (Math.pow(2, 8)) - 1) {
                return sprintf_js_1.sprintf('varbin field public_key too long %d/%d', this.public_key.length, (Math.pow(2, 8)) - 1);
            }
        }
        return;
    }
...
class OutputTag {
    ...
    Validate() {
        // tag (string)
        {
            if (this.tag && this.tag.length > (Math.pow(2, 8)) - 1) {
                return sprintf_js_1.sprintf('varchar field tag too long %d/%d', this.tag.length, (Math.pow(2, 8)) - 1);
            }
        }
        return;
    }
