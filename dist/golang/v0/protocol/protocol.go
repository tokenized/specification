...
import (
	...
	"github.com/tokenized/specification/dist/golang/v0/actions"
...
func WrapAction(action actions.Action, isTest bool) (envelope.BaseMessage, error) {
	...
	message := envelopeV1.NewMessage([]envelopeBase.ProtocolID{GetProtocolID(isTest)},
...
func SerializeFlagOutputScript(flag []byte) (bitcoin.Script, error) {
	message := envelopeV1.NewMessage([]envelopeBase.ProtocolID{[]byte(FlagProtocolID)},
