...
type AssetDefinition struct {
	...
	AssetID                     [32]byte `json:"asset_id,omitempty"`                      // 32 randomly generated bytes.  Each Asset ID should be unique.  However, an Asset ID is always linked to a Contract that is identified by the public address of the Contract wallet. The Asset Type + Asset ID = Asset Code.  An Asset Code is a human readable identifier that can be used in a similar way to a Bitcoin (BSV) address.
...
func (action *AssetDefinition) serialize() ([]byte, error) {
	...
	if err := write(buf, action.AssetID); err != nil {
		return nil, err
	}
...
func (action *AssetDefinition) write(b []byte) (int, error) {
	...
	if err := read(buf, &action.AssetID); err != nil {
		return 0, err
	}
