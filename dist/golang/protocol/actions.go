...
type AssetDefinition struct {
	...
	TradeRestrictions           Polities  `json:"trade_restrictions,omitempty"`            // Asset can only be traded within the trade restrictions.  Eg. AUS - Australian residents only.  EU - European Union residents only.
...
func (m *AssetDefinition) Validate() error {
	...
	// TradeRestrictions (Polities)
	{
		if err := m.TradeRestrictions.Validate(); err != nil {
			return fmt.Errorf("field TradeRestrictions is invalid : %s", err)
		}
...
func (m *AssetModification) Validate() error {
	...
	// Modifications ([]Amendment)
	{
		if len(m.Modifications) > (2<<0)-1 {
			return fmt.Errorf("list field Modifications has too many items %d/%d", len(m.Modifications), (2<<0)-1)
		}

		for i, value := range m.Modifications {
			err := value.Validate()
			if err != nil {
				return fmt.Errorf("list field Modifications[%d] is invalid : %s", i, err)
			}
		}
	}
